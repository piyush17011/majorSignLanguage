...
const predictionTexts = document.getElementById("predictionText");
...
socket.on("joined", (allusers) => {
    ...
    // Send the latest caption to the new user
    if (latestPrediction) {
        // When detecting a new caption
        socket.emit("caption", { caption: latestPrediction });

    }
});

// Handle caption updates from server
socket.on("caption", (caption) => {
    console.log("Caption received via WebSocket:", caption);
    predictionTexts.innerText = `Caption: ${caption}`;
    latestPrediction = caption;  // Store latest caption
});

// Handle new user receiving the caption


socket.on("receive-caption", (caption) => {
